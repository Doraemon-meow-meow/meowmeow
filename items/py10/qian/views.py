...
def info(request):
	...
	return render(request,'qian/info.html')


#列表页
def list(request):
	
	return render(request,'qian/list.html')


#注册页
def register(request):
	if request.method == 'GET':
		return render(request,'qian/register.html')
	elif request.method == 'POST':
		#先判断验证码是否正确
		if request.POST['yanzhen'].upper() != request.session['verifycode'].upper():
...
def login(request):
	if request.method =="GET":
		return render(request,'qian/login.html')
	elif request.method == "POST":
		#执行登录
		...
		try:
			...
			pass
		return HttpResponse('<script>alert("账号或者密码错误");location.href="/"</script>')
...
def tlogin(request):
	...
	return HttpResponse('<script>alect("退出成功");location.href="/"</scrit>')