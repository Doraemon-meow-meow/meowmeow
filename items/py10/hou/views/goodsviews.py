...
def add(request):
    if request.method == 'GET':
        ...
        return render(request,'hou/goods/add.html',context)
    elif request.method == 'POST':

        # 先判断是否右图片上传
        if not request.FILES.get('pic',None):
            return HttpResponse('<script>alert("必须选择商品图片");location.href="'+reverse('hou2_add')+'"</script>')
        pic = uploads(request)
        if pic == 1:
            return HttpResponse('<script>alert("图片类型错误");location.href="'+reverse('hou2_add')+'"</script>')
        ...
        return HttpResponse('<script>alert("添加成功");location.href="'+reverse('hou2_list')+'"</script>')
...
def index(request):
    ...
    if types:
        ...
        if types == 'all':
            try:
                keywords1 = Types.objects.get(name=keywords).id
                #全条件搜索
                #select * from user where username like '%aa%' 
                from django.db.models import Q
                userlist = Goods.objects.filter(
            ...
            # 按照title搜索
            userlist = Goods.objects.filter(title__contains=keywords)

        elif types == 'descr':
            userlist = Goods.objects.filter(descr__contains=keywords)

        elif types == 'price':
            userlist = Goods.objects.filter(price__contains=keywords)

        elif types == 'fname':
            keywords1 = Types.objects.get(name=keywords).id
            userlist = Goods.objects.filter(typeid=keywords1)

        elif types == 'status':

            userlist = Goods.objects.filter(status__contains=keywords)
    ...
    return render(request,'hou/goods/list.html',context)
...
def edit(request):
    ...
    if request.method == 'GET':
        ...
        # 分配数据
        context = {'uinfo':ob,'ob1':ob1}
        # 显示编辑页面
        return render(request,'hou/goods/edit.html',context)
        ...
        ob2 = Types.objects.get(id=request.POST['type'])
        ob.typeid = ob2
        ob.save()

        return HttpResponse('<script>alert("更新成功");location.href="'+reverse('hou2_list')+'"</script>')
        # except:
