...
def add(request):
	...
	if request.method == 'GET':
		#显示添加页面
		return render(request,'hou/user/add.html')

	elif request.method == 'POST':
		# 执行用户数据添加
		try:
			# 接受表单提交的数据 copy()拷贝 dict()转化为字典格式
			...
			if request.FILES.get('pic',None):
				...
				if data['pic'] ==1:
					return HttpResponse('<script>alert("上传的文件类型不符合要求");location.href="'+reverse('hou_add')+'"</script>')
			...
			return HttpResponse('<script>alert("添加成功");location.href="'+reverse('hou_list')+'"</script>')

		except:
			return HttpResponse('<script>alert("添加失败");location.href="'+reverse('hou_add')+'"</script>')
...
def index(request):
	...
	# 加载模板
	return render(request,'hou/user/list.html',context)
...
def edit(request):
    ...
    if request.method == 'GET':
        ...
        context = {'uinfo':ob}
        # 显示编辑页面
        return render(request,'hou/user/edit.html',context)
        ...
        try:
            ...
            return HttpResponse('<script>alert("更新成功");location.href="'+reverse('hou_list')+'"</script>')
        except:
